...
# utility methods
def addRecord(enrollment_id, name, address, phone_no, dob, department, semester):
...
def deleteRecord(search_field, key):
    temp = []
...
def updateRecord(search_field, key, update_field, update_value):
    temp = []
    f_obj = open('Student.csv', 'r')
    f_reader = csv.DictReader(f_obj)
    try:
...
def main():
    ...
    while end == 1:
        ...
        if choice == 1:
            ...
            addRecord(enrollment_id, name, address, phone_no, dob, department, semester)
            print("\nData added")
            ...
            search_field = input("\nEnter the column name(search field) on the basis of which you want" +
                                 "to delete record : ")
            if search_field not in col_name:
                print("\nWrong column name !!!")
                return
            key = input("\nEnter the value for the search field : ")
            deleteRecord(search_field, key)
            ...
            updateRecord(search_field, key, update_field, update_value)
